...
#
# DESCRIPTION
# This script should be in the directory where calc3p or calc3a.exe is located
...
do
  ...
  execute "$f" > calc3a.out 2> calc3a.err

  # Check diff's return value
  if [ -r "$expected_err" ]; then
    echo " --> Checking for expected errors..."
    if diff -u "$expected_err" calc3a.err > calc3a.err.diff; then
      success "$testname"
    else
      failure "$testname"
      echo "  --> Diff results:"
      showdiff calc3a.err.diff
    fi
  else
    echo " --> Checking output..."
    if diff -u "$expected" calc3a.out > calc3a.out.diff; then
      # Output was correct but there may have been extra errors.
      if [ -s calc3a.err ]; then
...
rm -f calc3a.out calc3a.err calc3a.out.diff calc3a.err.diff
